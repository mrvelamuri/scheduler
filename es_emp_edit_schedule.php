...
if (empty($s_group) && empty($s_exptime)) {
	if (empty($create)) {
		...
		for($i=0; $i<7; $i++) {
			...
			$schedules[$i]["s_group"] = $user["u_name"]."|".$schedules[0]["s_starttime"];
		}
	}
}
else {
	...
	if (!check_user_schedules($starttime, $exptime, $schedules[0]["s_group"], $user)) {
		...
		foreach($schedules as $schedule) {
			...
			if (empty($schedule["s_id"])) {
				...
				$sql = "INSERT INTO es_schedule VALUES (NULL, ".$user["u_id"].", 0, '".addslashes($user["u_name"])."', $starttime, '".$schedule["s_hours"]."', $repeat, $exptime, '', NULL)";
				$res = dbquery($sql);
		...
		if ((!empty($sendemails))&&($sendemails=="yes")) {
			...
			foreach($supervisors as $supervisor) {
				if ((!empty($supervisor["u_email"]))&&(strpos($supervisor["u_email"], "@")!==false)) {
					$headers="";
					$message = $es_lang["hello"]." ".$supervisor["u_name"].",\n\n".$es_lang["email_msg1"].$user["u_name"].$es_lang["email_msg2"]."\n".$SITE_URL."es_sup_employee_schedule.php?u_id=".$user["u_id"]."\n\n".$es_lang["email_msg3"]." ".$user["u_name"].".\n\n";
					$subject = $user["u_name"]." ".$es_lang["email_subject"];
					if (!empty($user["u_email"])) {
						if ($ES_FULL_MAIL_TO) $headers = "From: ".$user["u_name"]." <".$user["u_email"].">\r\n";
						else $headers = "From: ".$user["u_email"]."\r\n";
					}
					if ($ES_FULL_MAIL_TO) $to = $supervisor["u_name"]." <".$supervisor["u_email"].">";
					else $to = $supervisor["u_email"];
	...
	function setcell(cell, i, j) {
		...
		if (hour) {
			...
			//-- if the old value was a preference level and the new value is unavailable then add to the hours list
				...
				document.scheduleform.maxleft.value = parseFloat(document.scheduleform.maxleft.value) + 1/<?php print $sections_in_hour; ?>;
				document.scheduleform.totalhours.value = parseFloat(document.scheduleform.totalhours.value) - 1/<?php print $sections_in_hour; ?>;
				if (document.scheduleform.totalhours.value < <?php print $user["u_min"]?>) document.scheduleform.minleft.value = parseFloat(document.scheduleform.minleft.value) + 1/<?php print $sections_in_hour; ?>;
			}
		}
	}
...
<td>
	<table>
	<tr><td align="right"><?php print $es_lang["minimum_hours"]; ?></td><td> <?php print $user["u_min"]?></td></tr>
	<tr><td align="right"><?php print $es_lang["maximum_hours"]; ?></td><td> <?php print $user["u_max"]?></td></tr>
	<tr><td align="right"><?php print $es_lang["min_left"]; ?></td><td><input type="text" name="minleft" value="<?php if ($user["u_min"]-$total_hours < 0) print "0"; else print ($user["u_min"]-$total_hours);?>" readonly size="2" style="border: none; font-size: 12pt;"></td></tr>
	<tr><td align="right"><?php print $es_lang["max_left"]; ?></td><td><input type="text" name="maxleft" value="<?php print $user["u_max"]-$total_hours?>" readonly size="2" style="border: none; font-size: 12pt;"></td></tr>
	...
	<tr><td align="right"><b><?php print $es_lang["hours_desired"]; ?></b></td><td><input type="text" name="totaldesired" value="<?php print $user["u_hours"]?>" readonly size="2" style="border: none; font-size: 12pt; font-weight: bold;"></td></tr>
