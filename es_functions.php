...
function get_supervisor_areas($user) {
	$sql = "SELECT * FROM es_area, es_area_sups WHERE a_id=as_a_id AND as_u_id=".$user["u_id"]." ORDER BY a_name";
...
function get_supervisor_employees($user) {
	...
	$sql = "SELECT * FROM es_user, es_user_sups WHERE us_emp_id=u_id AND us_sup_id=".$user["u_id"]." ORDER BY u_name";
	$users = get_db_items($sql);
	for($i=0; $i<count($users); $i++) {
		if (empty($users[$i]["u_name"])) $users[$i]["u_name"]="No Name";
		if (!empty($users[$i]["u_id"])) $users_cache[$users[$i]["u_id"]] = $users[$i];
	}
...
function get_employee_supervisors($user) {
	...
	if (empty($user["u_id"])) return array();
	$sql = "SELECT * FROM es_user, es_user_sups WHERE us_sup_id=u_id AND us_emp_id=".$user["u_id"]." ORDER BY u_name";
	$users = get_db_items($sql);
	for($i=0; $i<count($users); $i++) {
		if (empty($users[$i]["u_name"])) $users[$i]["u_name"]="No Name";
		if (!empty($users[$i]["u_id"])) $users_cache[$users[$i]["u_id"]] = $users[$i];
...
function get_supervisors() {
	...
	$sql = "SELECT * FROM es_user WHERE u_type LIKE '%Supervisor%' OR u_type LIKE '%Admin%' ORDER BY u_name";
	$users = get_db_items($sql);
	for($i=0; $i<count($users); $i++) {
		if (empty($users[$i]["u_name"])) $users[$i]["u_name"]="No Name";
		if (!empty($users[$i]["u_id"])) $users_cache[$users[$i]["u_id"]] = $users[$i];
...
function get_admins() {
	...
	$sql = "SELECT * FROM es_user WHERE u_type LIKE '%Admin%' ORDER BY u_name";
	$users = get_db_items($sql);
	for($i=0; $i<count($users); $i++) {
		if (empty($users[$i]["u_name"])) $users[$i]["u_name"]="No Name";
		if (!empty($users[$i]["u_id"])) $users_cache[$users[$i]["u_id"]] = $users[$i];
...
function get_user_schedules($user) {
	...
	if (isset($user_cache[$user["u_id"]]["u_schedules"])) return $user_cache[$user["u_id"]]["u_schedules"];
	
	$sql = "SELECT * FROM es_schedule WHERE s_u_id=".$user["u_id"]." AND s_exptime>".time()." ORDER BY s_starttime";
	...
	for($i=0; $i<$ct; $i++) {
		if (!isset($schedules[$i])) {
			...
			$schedules[$i]["s_group"] = $user["u_name"];
		}
		else {
			...
			$sql = "SELECT es_position_assignment.* FROM es_position_assignment, es_schedule WHERE pa_u_id=".$user["u_id"]." AND pa_s_id=s_id AND pa_us_id=".$schedules[$i]["s_id"];
	...
	if (!isset($user_cache[$user["u_id"]])) $user_cache[$user["u_id"]] = $user;
	$user_cache[$user["u_id"]]["u_schedules"] = $schedules;
	return $schedules;
}
...
function get_past_user_schedules($user) {
	...
	for($i=0; $i<$ct; $i++) {
		...
		$sql = "SELECT es_position_assignment.* FROM es_position_assignment, es_schedule WHERE pa_u_id=".$user["u_id"]." AND pa_s_id=s_id AND pa_us_id=".$schedules[$i]["s_id"]." AND s_exptime>".time()." AND s_starttime<".time();
		$assignments = get_db_items($sql);
		$schedules[$i]["s_assignments"] = array();
		foreach($assignments as $assignment) {
			$schedules[$i]["s_assignments"][$assignment["pa_hour"]] = $assignment;
...
function get_timed_user_schedules($user, $starttime, $exptime) {
	$sql = "SELECT * FROM es_schedule WHERE s_u_id=".$user["u_id"]." AND ((s_exptime<=$exptime AND s_exptime>$starttime) OR (s_starttime>=$starttime AND s_starttime<$exptime)) ORDER BY s_starttime";
	$schedules = get_db_items($sql);
	$ct = count($schedules);
	for($i=0; $i<$ct; $i++) {
		$sql = "SELECT * FROM es_schedule_comment WHERE sc_s_id=".$schedules[$i]["s_id"];
...
function get_next_user_starttime($user) {
	$sql = "SELECT MAX(s_exptime) FROM es_schedule WHERE s_u_id=".$user["u_id"]." ORDER BY s_starttime";
...
function check_user_schedules($starttime, $exptime, $s_group, $user) {
	$sql = "SELECT * FROM es_schedule WHERE s_u_id=".$user["u_id"]." AND s_group!='".addslashes($s_group)."' AND ((s_exptime>$starttime AND s_exptime<=$exptime) OR (s_starttime>=$starttime AND s_starttime<$exptime)) ORDER BY s_starttime";
	$schedules = get_db_items($sql);
...
function get_user_schedule_group($group, $exptime, $user) {
	$sql = "SELECT * FROM es_schedule WHERE s_u_id=".$user["u_id"]." AND s_group='".addslashes($group)."' AND s_exptime=$exptime ORDER BY s_starttime, s_id";
	$schedules = get_db_items($sql);
...
function get_area_supervisors($a_id) {
	...
	$sql = "SELECT * FROM es_user, es_area_sups WHERE as_a_id=$a_id AND u_id=as_u_id ORDER BY u_name";
...
function get_user($u_id, $ignore_cache=false) {
	...
	if (!$ignore_cache && isset($users_cache[$u_id])) $user = $users_cache[$u_id];
		...
		$sql = "SELECT * FROM es_user WHERE u_id=$u_id";
		$users = get_db_items($sql);
		if (count($users)==0) return false;
		$user = $users[0];
	}
	if (empty($user["u_id"])) return false;
	if (empty($user["u_name"])) $user["u_name"]="No Name";
	if (empty($user["u_min"])) $user["u_min"] = 20;
	if (empty($user["u_color"])) $user["u_color"] = "#".dechex(rand(0,15)).dechex(rand(0,15)).dechex(rand(0,15)).dechex(rand(0,15)).dechex(rand(0,15)).dechex(rand(0,15));
...
function delete_user($u_id) {
	...
	if ($user) {
		...
		$sql = "DELETE FROM es_user WHERE u_id=$u_id";
		$res = dbquery($sql);
...
function auth_user() {
	...
	if ((!empty($_SESSION["es_username"]))&&(empty($_POST["username"]))) {
		$sql = "SELECT * FROM es_user WHERE u_netid='".$_SESSION["es_username"]."'";
		$res = dbquery($sql);
		if (mysql_num_rows($res)>0) {
		...
		$sql = "SELECT * FROM es_user WHERE u_netid='".$_POST["username"]."'";
		$res = dbquery($sql);
		if (mysql_num_rows($res)>0) {
			$user = db_cleanup(mysql_fetch_array($res));
			if (empty($user["u_min"])) $user["u_min"] = 20;
			...
			if (preg_match("/LDAP/", $user['u_type'])==0) {
				if (crypt($_POST["userpass"], $user['u_password'])==$user['u_password']) {
					$_SESSION["es_username"] = $_POST["username"];
					$_SESSION["u_type"] = $user["u_type"];
					if (!empty($user["u_language"])) {
						$_SESSION["CLANGUAGE"] = $user["u_language"];
			...
			//-- for ldap employees check ldap_auth
				...
				if (!isset($DOUBLE_AUTH)) {
					...
					$attrs = ldap_auth($LDAP_HOST, $LDAP_PORT, $LDAP_SEARCHBASE, "uid=".$user['u_netid'], $_POST["userpass"], "ou=people,o=byu.edu", "uid=".$user["u_netid"], $LDAP_ATTRS_ARRAY);
					if ($attrs!==false) {
						$_SESSION["es_username"] = $_POST["username"];
						$_SESSION["u_type"] = $user["u_type"];
						if (!empty($user["u_language"])) {
...
function auth_supervisor() {
	...
	$user = auth_user();
	if (preg_match("/(Supervisor)|(Admin)/i", $user["u_type"])==0) show_error_page($es_lang["access_denied"]);
	return $user;
}
...
function auth_admin() {
	...
	$user = auth_user();
	if (preg_match("/Admin/i", $user["u_type"])==0) show_error_page($es_lang["access_denied"]);
	return $user;
}
...
<?php
		...
		if ($user) {
			...
			if (preg_match("/(Supervisor)|(Admin)/i", $user["u_type"])>0) {
...
function print_footer() {
	global $VERSION, $TOTAL_QUERIES, $view, $COMPANY_NAME, $ES_SHOW_STATS, $EXEC_START_TIME;
?>
<br /><br /><br /><br />
<?php 
include "es_footer.html";
if ($view!="print") { ?>
	<p class="version"><a href="http://empscheduler.sourceforge.net/" target="_blank">Employee Scheduler Version <?php print $VERSION?></a><br />
	<?php if ($ES_SHOW_STATS) {
...
<tr valign="top"> 
  <td height="100%" bgcolor="F0F0E2">
	...
	<br />
	<span class='navtitle'>
	<?php if (!empty($user["u_picture"])) print '<img align="middle" src="photos/'.$user["u_picture"].'" width="40"> '; else print $es_lang["welcome"];?>
	<?php print $user["u_name"];?></span><br /><br />
	<table>
	<tr><td>
	<a href='es_emp_index.php' class='navlink'><b><?php print $es_lang["my_schedule"]; ?></b></a>
	</td></tr><tr><td>
	<a href='es_emp_past_schedules.php' class='navlink'><b><?php print $es_lang["my_past_schedule"]; ?></b></a>
	</td></tr><tr><td>
...
<tr valign="top"> 
  <td height="100%" bgcolor="F0F0E2">
	...
	<a href='es_sup_reports.php' class='navlink'><b><?php print $es_lang["reports"]; ?></b></a>
	</td></tr><tr><td>
	...
	<a href='es_emp_past_schedules.php' class='navlink'><b><?php print $es_lang["my_past_schedule"]; ?></b></a>
	<?php if (strstr($user["u_type"], "Admin")) { ?>
	</td></tr><tr><td>
