...
if (empty($s_group) && empty($s_exptime)) {
	if (empty($create)) {
		...
		for($i=0; $i<7; $i++) {
			...
			$schedules[$i]["s_group"] = $employee["u_name"]."|".$schedules[0]["s_starttime"];
		}
	}
}
else {
	...
	if (!check_user_schedules($starttime, $exptime, $schedules[0]["s_group"], $employee)) {
		...
		foreach($schedules as $schedule) {
			...
			if (empty($schedule["s_id"])) {
				...
				$sql = "INSERT INTO es_schedule VALUES (NULL, ".$employee["u_id"].", 0, '".addslashes($employee["u_name"]."-$s_startdate")."', $starttime, '".$schedule["s_hours"]."', $repeat, $exptime, '', NULL)";
				$res = dbquery($sql);
		...
		if ((!empty($sendemails))&&($sendemails=="yes")) {
			if ((!empty($employee["u_email"]))&&(strpos($employee["u_email"], "@")!==false)) {
				$headers="";
				$message = $es_lang["hello"]." ".$employee["u_name"].",\n\n".$user["u_name"]." ".$es_lang["email_msg4"]."\n".$SITE_URL."es_emp_index.php\n\n".$es_lang["email_msg5"]." ".$user["u_name"].".\n\n";
				$subject = $employee["u_name"]." ".$es_lang["schedule_update"];
		...
		header("Location: es_sup_employee_schedule.php?u_id=$u_id&".session_name()."=".session_id());
	}
}
...
print_header("Edit Schedule - ".$employee["u_name"]);
?>
...
<script language="JavaScript" type="text/javascript">
	...
	function setcell(cell, i, j) {
		...
		if (newval==8) {
			...
			if (document.scheduleform.totalassigned.value < <?php print $employee["u_max"]?>) {
		...
		if ((curval == 0)&&(newval>0)) {
			...
			document.scheduleform.maxleft.value = parseFloat(document.scheduleform.maxleft.value) +1/<?php print $sections_in_hour; ?>;
			document.scheduleform.totalhours.value = parseFloat(document.scheduleform.totalhours.value) - 1/<?php print $sections_in_hour; ?>;
			if (document.scheduleform.totalhours.value < <?php print $employee["u_min"]?>) document.scheduleform.minleft.value = parseFloat(document.scheduleform.minleft.value) + 1/<?php print $sections_in_hour; ?>;
...
<br><br><span class="pagetitle"><?php print $es_lang["edit_schedule"];?> - <?php print $employee["u_name"];?></span><br>
...
<td colspan=3>
	<table>
	<tr><td align="right"><?php print $es_lang["minimum_hours"]; ?></td><td> <?php print $employee["u_min"]?></td><td align="right"><?php print $es_lang["min_left"]; ?></td><td><input type="text" name="minleft" value="<?php if ($employee["u_min"]-$total_hours < 0) print "0"; else print ($employee["u_min"]-$total_hours);?>" readonly size="2" style="border: none; font-size: 12pt;"></td></tr>
	<tr><td align="right"><?php print $es_lang["maximum_hours"]; ?></td><td> <?php print $employee["u_max"]?></td><td align="right"><?php print $es_lang["max_left"]; ?></td><td><input type="text" name="maxleft" value="<?php print $employee["u_max"]-$total_hours?>" readonly size="2" style="border: none; font-size: 12pt;"></td></tr>
	<tr><td align="right"><b><?php print $es_lang["total_hours"]; ?></b></td><td><input type="text" name="totalhours" value="<?php print $total_hours?>" readonly size="2" style="border: none; font-size: 12pt; font-weight: bold;"></td><td align="right"><b><?php print $es_lang["hours_desired"]; ?></b></td><td><input type="text" name="totaldesired" value="<?php print $employee["u_hours"]?>" readonly size="2" style="border: none; font-size: 12pt; font-weight: bold;"></td></tr>
			...
			if (isset($show_schedule)) {
				if (isset($schedule_group[$j]["s_assignments"][$i])) {
					foreach($schedule_group[$j]["s_assignments"][$i] as $assignment) {
						...
						print "<font style=\"color: #AA7766;\">".$auser["u_name"]."<br></font>";
					}
				}
			}
			print "</td>\n";
