...
if ((!empty($sendemails))&&($sendemails=="yes")) {
			...
			foreach($schedules[$j]["s_assignments"] as $hour_assignment) {
				foreach($hour_assignment as $assignment) {
					foreach($employees as $employee) {
						if ($assignment["pa_u_id"]==$employee["u_id"]){
	...
	foreach($email_employees as $employee) {
		if ((!empty($employee["u_email"]))&&(strpos($employee["u_email"], "@")!==false)) {
			...
			print "<br><b>".$es_lang["email_success"]." ".$employee["u_name"].".</b>\n";
		}
	}
}
...
<script language="JavaScript">
	...
	function open_employee_schedule(u_id, s_group, s_exptime) {
		window.open('es_sup_employee_simple_schedule.php?u_id='+u_id+'&s_group='+s_group+'&s_exptime='+s_exptime+'&<?php print session_name()."=".session_id(); ?>', '', 'left=50,top=50,width=600,height=600,scrollbars=1,resizable=1,menubar=1');
		return false;
	}
...
<td>
	...
	<?php
		...
		for($e=0; $e<count($employees); $e++) {
			...
			print "<tr><td align=\"right\" class=\"text\" style=\"background-color: ".$employee["u_color"].";\">".$employee["u_name"].": </td><td class=\"text\"><input type=\"text\" name=\"".$employee["u_id"]."_total\" value=\"$total_assigned\" readonly size=\"2\" style=\"border: none; font-size: 10pt; width: 25px;\"> of <input type=\"text\" name=\"".$employee["u_id"]."_max\" value=\"".$employee["u_max"]."\" readonly size=\"2\" style=\"border: none; font-size: 10pt; width: 15px;\"></td></tr>\n";
		}
	?>
	</table>
</td>
			...
			foreach($employees as $employee) {
				if (isset($employee["u_schedules"][$j])) {
					if (($employee["u_schedules"][$j]["s_hours"][$i]>0)&&($employee["u_schedules"][$j]["s_exptime"]>=$schedules[$j]["s_exptime"])) {
						...
						if ($pass) {
							print "<div style=\"white-space: nowrap; color: black; background-color: ".$employee["u_color"].";\" onmouseover=\"this.style.backgroundColor='".$PRIORITY[$employee["u_schedules"][$j]["s_hours"][$i]]."';\" onmouseout=\"this.style.backgroundColor='".$employee["u_color"]."';\"><input type=\"checkbox\" name=\"hour-".$i."-".$j."[]\" value=\"".$employee["u_id"]."\"";
							if (isset($schedules[$j]["s_assignments"][$i])) {
								foreach($schedules[$j]["s_assignments"][$i] as $assignment) {
									if ($assignment["pa_u_id"]==$employee["u_id"]) print " checked";
								}
							}
							print " onclick=\"return check_hours(".$employee["u_id"].", this);\">";
							print "<input type=\"hidden\" name=\"usid-".$i."-".$j."-".$employee["u_id"]."\" value=\"".$employee["u_schedules"][$j]["s_id"]."\">\n";
							print "<a href=\"#\" style=\"color: black;\"onclick=\"return open_employee_schedule(".$employee["u_id"].",'".$employee["u_schedules"][0]["s_group"]."','".$employee["u_schedules"][0]["s_exptime"]."');\">".$employee["u_name"]."- P".$employee["u_schedules"][$j]["s_hours"][$i]."</a></div>\n";
						}
					}
				}
			}
			if (isset($schedules[$j]["s_assignments"][$i])) {
				foreach($schedules[$j]["s_assignments"][$i] as $assignment) {
					...
					if (!$found) {
						...
						$employee["u_id"] = $assignment["pa_u_id"];
						print "<div style=\"white-space: nowrap; color: black;\">";
						print $employee["u_name"]." (".$employee["u_id"].")</div>\n";
					}
				}
			}
			print "</td>\n";
