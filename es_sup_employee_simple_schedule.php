...
print_simple_header($es_lang["emp_schedule"]." - ".$employee["u_name"]);
...
if ($action=="week") {
if ($view!="print") {
	print '<br>'.$es_lang["week"].' | <a href="es_sup_employee_simple_schedule.php?u_id='.$u_id.'&action=list">'.$es_lang["list"].'</a> ';
...
//-- show the schedule in a list view
	...
	if ($view!="print") {
		print '<br><a href="es_sup_employee_simple_schedule.php?u_id='.$u_id.'&action=week">'.$es_lang["week"].'</a> | '.$es_lang["list"].' ';
		//print '| <a href="es_emp_index.php?action=calendar">Calendar</a>';
		print '<br>';
	}
	$curj = -1;
